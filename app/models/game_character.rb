class GameCharacter < ActiveRecord::Base
  ...
  validate do |character|
    ...
    character.errors.add_to_base('服务器不能为空') if character.game and character.server_id.blank?
    character.errors.add_to_base('种族不能为空') if character.game and character.race_id.blank?
    character.errors.add_to_base('职业不能为空') if character.game and character.profession_id.blank?
  end
