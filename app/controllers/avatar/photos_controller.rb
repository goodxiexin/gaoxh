class Avatar::PhotosController < ApplicationController

  layout 'user'

  before_filter :login_required, :setup

  before_filter :owner_required, :only => [:edit, :set, :update, :update_notation, :destroy]
  ...
  def create
    if @photo = @album.photos.create(params[:photo]) 
        ...
        render :update do |page|
          ...
          if params[:album].to_i == 1
            ...
            page.replace_html 'avatar', album_cover(@album, :medium)
          end
        end
      end
    else
  ...
  def update
    if @photo.update_attributes(params[:photo])
      render :update do |page|
        page << "facebox.close();"
				page << "$('avatar_notation_#{@photo.id}').innerHTML = '#{@photo.notation}';"
      end
    else
