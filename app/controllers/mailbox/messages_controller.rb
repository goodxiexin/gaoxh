class Mailbox::MessagesController < ApplicationController
  ...
  def show
    Mail.update_all("read_by_receiver = 1", {:parent_id => @root_mail.id, :receiver_id => current_user.id})
  end
...
protected
  ...
  def catch_receiver
    @receiver = User.find(params[:receiver_id]) unless params[:receiver_id].blank?
  rescue
    not_found
  end
