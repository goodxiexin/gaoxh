EmotionConverter = Class.create({
  ...
  show_faces: function(link){
    ...
    for(var i=0;i<this.rows;i++){
      ...
      for(var j=0;j<this.faces_per_row && i*this.faces_per_row + j < this.faces_count;j++){
        ...
        var face = new Element('img', {src: "/faces/"+(i*this.faces_per_row + j)+".gif", width: '20px', height: '20px', index: (i*this.faces_per_row + j)});
  ...
  convert: function(text_area){
    for(var i=0;i<this.symbols.length;i++){
      var regex = new RegExp(this.escape(this.symbols[i]), 'g');
      text_area.innerHTML = text_area.innerHTML.replace(regex, "<img src='/faces/" + i + ".gif' width=20px height=20px />");
    }
  },

  convert_all: function(){
    var text_areas = $$('emotion_text');
    var size = text_areas.size();
    for(var i=0;i<size;i++){
      this.convert(text_areas[i]);
    }
  },
});
